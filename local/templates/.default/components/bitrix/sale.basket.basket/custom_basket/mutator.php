...
/*
 * Вывод сообщения, если сумма товаров меньше требуемой
 * и передача в элемент массива $this->arParams['DISABLE_CHECKOUT'] = true; параметра для отображения кнопки
 * так как способ с $totalData['DISABLE_CHECKOUT'] = true  не работал правильно в визуальной части
 * было принято решение не выводить вовсе кнопку при невыполнения условия
 * (на $totalData['DISABLE_CHECKOUT'] = true; поставил комментарии,
 * чтобы показать своё ознакомление с эталонным решением из официальной документации)
*/
if ($result['allSum'] < 2000) {
	$totalData['DISABLE_CHECKOUT'] = true; //из решения из документации
	echo "<h3>" . Loc::getMessage("SBB_LESS_THAN_REQUIRED") . "</h3>"; //реализация мультиязыковости
}
